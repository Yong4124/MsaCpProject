...
async function loadApplicants(e) {
    ...
    try {
        ...
        // ✅ 디버깅: 어떤 값이 선택되었는지 확인
        console.log('선택한 status 값:', statusValue);

        const url = statusValue !== ''
            ? `/api/jobs/${currentJobId}/applicants?status=${statusValue}`
        ...
        // ✅ 디버깅: 받아온 데이터 확인
        console.log('받아온 지원자 수:', applicants.length);
...
function downloadResumeFile(app) {
    ...
    // ✨ 경력 정보 수집 (다중)
    let careerHtml = '';
    if (app && app.careerHistories && app.careerHistories.length > 0) {
        app.careerHistories.forEach((career, idx) => {
            careerHtml += `
                <div style="margin-bottom: 15px;">
                    <p><b>[경력 ${idx + 1}]</b></p>
                    <p><b>회사명:</b> ${career.company || '-'} (${career.position || '-'})</p>
                    <p><b>부서/연봉:</b> ${career.department || '-'} / ${career.salary || '-'}</p>
                    <p><b>근무기간:</b> ${career.joinDate || '-'} ~ ${career.retireDate || '-'}</p>
                    <p><b>담당업무:</b> ${career.positionSummary || '-'}</p>
                    <p><b>경력기술서:</b> ${career.experience || '-'}</p>
                </div>
            `;
        });
    } else {
        careerHtml = '<p>등록된 경력 정보 없음</p>';
    }
