(() => {
    ...
    async function postForm(endpoint, extra = {}) {
        ...
        const KEY_MAP = {
            ...
            // ✅✅✅ 수정: hidden 경로 3개도 전송되게 KEY_MAP에 추가
        ...
        // ✅✅✅ 수정: hidden 3개는 "중복 append" 방지 위해 set으로 확정
        // (폼에 값이 없어서 ""로 들어오는 경우도 있으니 지금 DOM 값으로 한번 더 세팅)
    ...
    async function init() {
        ...
        // ✅✅✅ 추가: 지원현황에서 resumeId를 들고 넘어오면 자동으로 해당 이력서 로드
        if (resumeId) {
            try {
