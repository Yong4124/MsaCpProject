(() => {
    ...
    async function autoFillBasicProfile() {
        try {
            ...
            const birth = member?.birth ?? member?.birthDate ?? member?.birth_date ?? member?.birthDay ?? "";
    ...
    `;
        el.querySelector(".apply_btn_remove").addEventListener("click", () => el.remove());
        return el;
    }
    ...
    async function postForm(endpoint, extra = {}) {
        ...
        // ✅ ApplyRequest는 phone/email/... 소문자 필드라서 대문자 HTML 키만 변환
        const KEY_MAP = {
            "PHONE": "phone",
            "EMAIL": "email",
            "ADDRESS": "address",
        ...
        for (const [k, v] of rawFd.entries()) {
            ...
            // 컨트롤러가 CAREER_COMPANY(무[])도 받게 돼있어서 []는 제거
    ...
    function toResumeModel(raw) {
        ...
        return {
            ...
            school: data?.school ?? "",
            major: data?.major ?? "",
            enrollDate: data?.enrollDate ?? data?.enroll_date ?? "",
            graduateDate: data?.graduateDate ?? data?.graduate_date ?? "",
            gpa: data?.gpa ?? "",
            graduateStatus: data?.graduateStatus ?? data?.graduate_status ?? "",

            skill: data?.skill ?? "",
            selfIntro: data?.selfIntro ?? data?.self_intro ?? "",

            careers: careers.map((c) => ({
                company: c?.company ?? "",
                department: c?.department ?? "",
                joinDate: c?.joinDate ?? c?.join_date ?? "",
                retireDate: c?.retireDate ?? c?.retire_date ?? "",
                position: c?.position ?? "",
                salary: c?.salary ?? "",
                positionSummary: c?.positionSummary ?? c?.position_summary ?? "",
    ...
    `;
        ...
        wrap.innerHTML = `
      <div style="width: 520px; max-width: calc(100vw - 24px); background: #fff; border-radius: 10px; overflow: hidden;">
        <div style="padding: 12px 14px; display:flex; align-items:center; justify-content:space-between; border-bottom: 1px solid #eee;">
          <div style="font-weight: 700;">이력서 선택</div>
          <button type="button" id="resumeModalClose" style="border:0;background:transparent;font-size:18px;cursor:pointer;">✕</button>
        </div>

        <div style="padding: 12px 14px;">
          <div id="resumeModalList" style="display:flex; flex-direction:column; gap:10px;"></div>

          <div id="resumeModalEmpty" style="display:none; padding: 14px; text-align:center; color:#777;">
            저장된 이력서가 없습니다.
          </div>

          <div style="display:flex; justify-content:space-between; align-items:center; margin-top:12px;">
            <button type="button" id="resumePrevBtn">이전</button>
            <div id="resumePageInfo" style="font-size:13px; color:#555;"></div>
            <button type="button" id="resumeNextBtn">다음</button>
          </div>

          <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:14px;">
            <button type="button" id="resumeLoadSelectedBtn">선택 불러오기</button>
            <button type="button" id="resumeLoadLatestBtn">최신 1개 불러오기</button>
          </div>
        </div>
      </div>
    `;
    ...
    async function loadLatestResumeAndBind() {
    ...
    async function openResumePicker() {
        ...
        const render = async () => {
            $("#resumeModalList").innerHTML = "";
            $("#resumeModalEmpty").style.display = "none";
            $("#resumePageInfo").textContent = "";

            // 목록 API가 없거나 실패하면 "최신 1개"로 fallback
            let raw;
            try {
                raw = await fetchJson(ENDPOINT_RESUME_LIST(page, size));
            } catch (e) {
                // fallback: 최신 1개만
                $("#resumeModalEmpty").style.display = "block";
                $("#resumeModalEmpty").textContent = "목록 기능이 아직 준비되지 않았습니다. '최신 1개 불러오기'를 사용하세요.";
                $("#resumePrevBtn").disabled = true;
                $("#resumeNextBtn").disabled = true;
                $("#resumePageInfo").textContent = "";
                return;
            }
            ...
            $("#resumePageInfo").textContent = totalPages ? `${curPage + 1} / ${totalPages}` : "";

            $("#resumePrevBtn").disabled = curPage <= 0;
            $("#resumeNextBtn").disabled = totalPages ? curPage >= totalPages - 1 : true;

            if (!items.length) {
                $("#resumeModalEmpty").style.display = "block";
                $("#resumeModalEmpty").textContent = "저장된 이력서가 없습니다.";
                return;
            }
            ...
            items.forEach((it, idx) => {
                ...
                row.style.cssText = "display:flex; gap:10px; padding:10px; border:1px solid #eee; border-radius:8px; cursor:pointer; align-items:flex-start;";
                row.innerHTML = `
          <input type="radio" name="resumePick" value="${m110}" ${idx === 0 ? "checked" : ""} />
          <div style="display:flex; flex-direction:column; gap:4px;">
            <div style="font-weight:700;">이력서 #${m110}</div>
            <div style="font-size:13px; color:#666;">저장일: ${insertDate}</div>
            <div style="font-size:13px; color:#666;">${school}${major ? " / " + major : ""}</div>
          </div>
        `;
                $("#resumeModalList").appendChild(row);
            });
        };

        $("#resumePrevBtn").onclick = async () => {
            page = Math.max(page - 1, 0);
            await render();
        };

        $("#resumeNextBtn").onclick = async () => {
            page = page + 1;
            await render();
        };

        $("#resumeLoadLatestBtn").onclick = async () => {
            try {
                await loadLatestResumeAndBind();
                closeModal();
                alert("이력서 불러오기 완료");
            } catch (e) {
                console.error(e);
        ...
        $("#resumeLoadSelectedBtn").onclick = async () => {
            try {
                const picked = document.querySelector('input[name="resumePick"]:checked')?.value;
                if (!picked) {
                    alert("이력서를 선택하세요.");
                    return;
                }
                const raw = await fetchJson(ENDPOINT_RESUME_ONE(picked));
                const resume = toResumeModel(raw);
                bindResume(resume);
                await autoFillBasicProfile();
                closeModal();
    ...
    async function init() {
        ...
        $("#tempSaveBtn")?.addEventListener("click", async () => {
            try {
                await postForm(ENDPOINT_TEMP, {
                    // ✅ 서버 ApplyRequest는 SEQ_NO_M210 이걸로 jobId 받음
                    SEQ_NO_M210: $("#SEQ_NO_M210").value,
                });
                alert("임시 저장되었습니다.");
            } catch (e) {
        ...
        $("#applyForm")?.addEventListener("submit", async (e) => {
            e.preventDefault();
            try {
                await postForm(ENDPOINT_SUBMIT, {
                    SEQ_NO_M210: $("#SEQ_NO_M210").value,
                });
                alert("제출 완료되었습니다.");
                location.href = "/jobs";
...
// ✅ 팝업에서 선택한 이력서 받기
    window.addEventListener("message", async (e) => {
        ...
        try {
            const raw = await fetchJson(`/api/resume/${m110}`); // ✅ 단건 불러오기
            const resume = toResumeModel(raw);
            bindResume(resume);
            await autoFillBasicProfile();
            alert("이력서 불러오기 완료");
